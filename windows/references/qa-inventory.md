...
3. All real Codex controls remain interactive; the cultivation layer is not a screenshot overlay.
4. The cultivation layer survives route changes and renderer reloads while the injector daemon runs.
...
- Pet overlay: open a desktop pet and confirm its auxiliary window stays transparent with no cultivation background or decoration layer behind it.
- Restore/reapply cycle: remove the live cultivation layer, verify marker absent, apply again, verify marker present.
