...
async function createFixture() {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "codex-cultivation-pet-"));
  return {
    root,
...
try {
  ...
  console.log("PASS: Silver Moon pet manager validates, evolves, protects collisions, and removes managed files safely.");
} finally {
