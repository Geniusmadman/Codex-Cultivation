...
function defaultStateRoot(env = process.env) {
  if (!env.LOCALAPPDATA) throw new Error("LOCALAPPDATA is required to manage the Silver Moon pet state");
  return path.resolve(env.LOCALAPPDATA, "CodexCultivation");
}

function defaultCodexHome(env = process.env) {
  if (env.CODEX_HOME) return path.resolve(env.CODEX_HOME);
  if (!env.USERPROFILE) throw new Error("USERPROFILE or CODEX_HOME is required to locate Codex pets");
  return path.resolve(env.USERPROFILE, ".codex");
}

export function defaultPetFamilyPath() {
  return path.join(windowsRoot, "pets", PET_ID, "pet-family.json");
}
