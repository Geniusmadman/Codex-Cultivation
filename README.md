...
- macOS：完整修仙界面、状态机、CC Switch 校准和美术资源，支持签名校验、暂停、背景导入、菜单栏控制与完整恢复。
...
Windows 的银月管理状态独立保存在 `%LOCALAPPDATA%\CodexCultivation\pet-state.json`，宠物安装到 `%USERPROFILE%\.codex\pets\yinyue`，不会修改其他宠物。
